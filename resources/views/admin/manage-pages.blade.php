                ...
                <button type="button"
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div class="text-left">
                            ...
                            <p class="text-xs text-gray-400">{{ count($group['items']) }} section{{ count($group['items']) > 1 ? 's' : '' }}</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-3">
                        ...
                        <i id="chevron-{{ $groupKey }}" class="fas fa-chevron-down text-sm {{ $c['chevron'] }} transition-transform duration-200"></i>
                    </div>
                </button>
                        ...
                        @foreach($group['items'] as $section)
                            <a href="{{ route('admin.edit-section', \Illuminate\Support\Str::slug($section)) }}"
                               class="section-card group bg-white border border-gray-200 rounded-lg p-4 flex justify-between items-center hover:shadow-md transition-all duration-200 {{ $c['card'] }}">
                                <span class="text-sm font-semibold text-gray-700 group-hover:text-inherit transition-colors leading-tight pr-2">
                                    {{ $section }}
                                </span>
                                <div class="flex-shrink-0 bg-gray-50 p-2 rounded-lg text-gray-400 transition-colors duration-200 {{ $c['cog'] }}">
